                        ...
                        <?php if (mysqli_num_rows($result) > 0): ?>
                            <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($row['invoice_created']): ?>
                                            <span class="badge bg-success">
                                                <i class="fas fa-check-circle me-1"></i>Invoice already created
                                            </span>
                                        <?php else: ?>
                                            <div class="btn-group">
                                                <button onclick="showConvertToInvoiceModal(<?= $row['estimate_id'] ?>, '<?= htmlspecialchars($row['estimate_number']) ?>')" 
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p><strong>This is the finalized quotation. Do you want to edit before converting, or directly generate the invoice?</strong></p>
                <p class="text-muted">Quotation: <span id="quotationNumberSpan"></span></p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-outline-primary" onclick="editQuotation()">
                    <i class="fas fa-edit me-1"></i>Edit
                </button>
                <button type="button" class="btn btn-success" onclick="printCreateInvoice()">
                    <i class="fas fa-print me-1"></i>Print (Create Invoice)
                </button>
            </div>
        </div>
    </div>
...
// Convert to Invoice Modal Functions
let currentEstimateId = null;
let currentQuotationNumber = null;

function showConvertToInvoiceModal(estimateId, quotationNumber) {
    currentEstimateId = estimateId;
    currentQuotationNumber = quotationNumber;
    
    document.getElementById('quotationNumberSpan').textContent = quotationNumber;
...
function editQuotation() {
    if (currentEstimateId) {
        window.location.href = `quotation_edit.php?id=${currentEstimateId}`;
    }
}
...
function printCreateInvoice() {
    ...
    // Show loading state
    const printBtn = document.querySelector('#convertToInvoiceModal .btn-success');
    const originalText = printBtn.innerHTML;
    printBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Creating Invoice...';
    printBtn.disabled = true;
    
    // Send AJAX request to mark quotation as invoice
    fetch('convert_to_invoice.php', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            // Close modal
            const modal = bootstrap.Modal.getInstance(document.getElementById('convertToInvoiceModal'));
            modal.hide();
            
            // Show success message
            if (data.already_created) {
                alert('Invoice has already been created for this quotation.');
            } else {
                // Open print page
                window.open(`quotation_print.php?id=${currentEstimateId}`, '_blank');
